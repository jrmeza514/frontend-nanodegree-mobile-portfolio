...
/*
  These Functions Do not Need To be Defined Every Single
  Time We Change Pizza Size
*/
...
function sizeSwitcher (size) {
  ...
  function changePizzaSizes(size) {
    ...
    // This Will result in the same value regardless of the specific pizza
    var dx = determineDx(randomPizzaContainer[0], size);
    var newwidth = (randomPizzaContainer[0].offsetWidth + dx) + 'px';
    // Change Pizza Size Without Extra Computing
...
var resizePizzas = function(size) {
  ...
  // Removed All The Repetitive Code
...
function updatePositions() {
  ...
  // compute these values once per update
  var sct = document.body.scrollTop / 1250;
  var phases = [
    Math.sin(sct),
    Math.sin(sct + 1 ),
    Math.sin(sct + 2 ),
